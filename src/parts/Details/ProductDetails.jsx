    ...
    <section className="container mx-auto">
      <div className="flex flex-wrap my-4 md:my-12">
        <div className="w-full md:hidden px-4">
          ...
          <span className="text-xl">{formatCurrency(data.data.price)}</span>
        </div>
          ...
          <div className="slider">
              ...
              {data.data?.imgUrls?.map((item, index) => {
                return (
                  <div className="px-2" key={index}>
                    <div className={["item", slider === item ? "bg-gray-100 selected" : ""].join(" ")} onClick={() => setSlider(item)}>
            ...
            <div className="preview">
              <div className="item rounded-lg h-full overflow-hidden">
                <img src={slider} alt="front" className="object-cover w-full h-full rounded-lg" />
              </div>
            </div>
          </div>
        </div>
        <div className="flex-1 px-4 md:p-6">
          <h2 className="text-5xl font-semibold">{data.data.title}</h2>
          <p className=" mt-5 text-xl">{formatCurrency(data.data.price)}</p>
